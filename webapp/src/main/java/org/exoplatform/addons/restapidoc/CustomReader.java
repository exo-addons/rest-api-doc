...
//        System.out.println(sep + "CustomReader : swagger initialization" + sep);
//        System.out.println(sep + "before initialization");
//        System.out.println("Swagger : " + getSwagger().getTags());
...
//        System.out.println(sep + "Trying to get annotations from " + cls.getCanonicalName() + sep);
        Api api = (Api) cls.getAnnotation(Api.class);
//        System.out.println(sep + "Api null ? : " + (api == null));
        ...
        //if (readable || cls.getCanonicalName().toString().equals("org.exoplatform.tgigant.TestJar")) {
        if (readable) {

//            System.out.println(sep + "The api is readable" + sep);
...
//        System.out.println(sep+"Mid reading class"+sep);
...
//        System.out.println(sep + "This API has been read"+ sep);
        return getSwagger();
    }
    ...
    protected Parameter readImplicitParam(ApiImplicitParam param, Class<?> apiClass) {
        ...
        return ParameterProcessor.applyAnnotations(getSwagger(), p, apiClass, Arrays.asList(new Annotation[]{param}));
    }
    ...
    protected void readSwaggerConfig(Class<?> cls, SwaggerDefinition config) {
        ...
        if (!config.basePath().isEmpty()) {
            getSwagger().setBasePath(config.basePath());
        }

        if (!config.host().isEmpty()) {
            getSwagger().setHost(config.host());
        }

        readInfoConfig(config);

        for (String consume : config.consumes()) {
            getSwagger().addConsumes(consume);
        }

        for (String produce : config.produces()) {
            getSwagger().addProduces(produce);
        }

        if (!config.externalDocs().value().isEmpty()) {
            ExternalDocs externalDocs = getSwagger().getExternalDocs();
            if (externalDocs == null) {
                externalDocs = new ExternalDocs();
                getSwagger().setExternalDocs(externalDocs);
            }
    ...
    protected void readInfoConfig(SwaggerDefinition config) {
        ...
        io.swagger.models.Info info = getSwagger().getInfo();
        if (info == null) {
            info = new io.swagger.models.Info();
            getSwagger().setInfo(info);
        }
